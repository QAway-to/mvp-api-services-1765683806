...
// DISABLED: Admin API functionality is disabled - only webhooks are used
 ...
 * Handles different payload formats from Bitrix
 * @param {Object} event - Bitrix webhook event payload
 * @returns {string|null} Deal ID or null if not found
 */
function extractDealId(event) {
  // Try different locations where deal ID might be
  if (event.data && event.data.FIELDS && (event.data.FIELDS.ID || event.data.FIELDS.id)) {
    return String(event.data.FIELDS.ID || event.data.FIELDS.id);
  }
  if (event.FIELDS && (event.FIELDS.ID || event.FIELDS.id)) {
    return String(event.FIELDS.ID || event.FIELDS.id);
  }
  if (event.ID || event.id) {
    return String(event.ID || event.id);
  }
  return null;
}
...
async function handleDealUpdate(dealId, requestId) {
  ...
  if (decision.categoryMatch && decision.stageMatch && decision.shopifyOrderIdPresent) {
    ...
    // TODO: In future steps, create Shopify fulfillment here
    // For now, just log the trigger match
...
export default async function handler(req, res) {
  ...
  // Extract deal ID from payload (try multiple locations)
  ...
  if (!dealId) {
    ...
    const errorLog = {
      ...
      payloadKeys,
      payloadSample: JSON.stringify(event).substring(0, 500), // First 500 chars
      timestamp: new Date().toISOString()
    };
    console.log(JSON.stringify(errorLog));
    res.status(400).json({ error: 'Invalid event format: no deal ID found', payloadKeys });
    return;
  }
